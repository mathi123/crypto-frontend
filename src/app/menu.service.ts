...
export class MenuService {
    ...
    private rebuildMenu(context: Context) {
        ...
        if (context !== null) {
            const profile = new MenuItem(false, 'My profile', '/profile', 'settings');
            const logout = new MenuItem(false, 'Sign out', '/logout', 'power_settings_new');

            menu.push(new MenuItem(false, context.name, '', 'account_circle', [profile, logout]));
        }
